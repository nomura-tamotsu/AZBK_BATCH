...
#
# FILE      : SQLServerバックアップ事後処理(NAS保管)
#
# FILE      : uy_sqlsvr_backup_nas_save.ps1
#
# INPUT     :
#
#----------------------------------------------------------------------------
...
${PRG_NAME}         = "SQLServerバックアップ事後処理(NAS保管)"
${PRG_EXIT_CODE}    = 0
#--------------------------------------------
#  Variable definition
#--------------------------------------------
...
${NAS_DIRS}         = "${COM_NAS_BASE_DIRS}\${HOST_NMLO}\dbbackup"
...
#--------------------------------------------
# SQLServerバックアップジョブ事後処理
#--------------------------------------------
# 1.SQLServerバックアップ（NAS保管分）の削除
#--------------------------------------------
FC_LogWriter ${DEF_RTNCD_NML} "SQLServerバックアップ事後処理(前日バックアップ(NAS保管分)の削除処理)を開始します。"
# SQLServerバックアップ (基本ディレクトリ)存在チェック
if ( !(Test-Path ${NAS_DIRS}) ) {
    FC_LogWriter ${DEF_RTNCD_NML} "SQLServerバックアップ(基本ディレクトリ)[${NAS_DIRS}]を作成します。"
...
FC_LogWriter ${DEF_RTNCD_NML} "SQLServerバックアップ事後処理(前日バックアップ(NAS保管分)の削除処理)を終了します。"

#--------------------------------------------
# 2.SQLServerバックアップ（NAS保管処理）
#--------------------------------------------
FC_LogWriter ${DEF_RTNCD_NML} "SQLServerバックアップ事後処理(NAS保管処理)を開始します。"
${TG_FIND_FILES} = @(Get-ChildItem ${UNYO_SQL_FLBKUP_DIR})
if ( ${TG_FIND_FILES}.length -gt 0 ) {

    # SQLServerバックアップ先 当日分保管用NASディレクトリ存在チェック
    ${ACTDAY_DB_BKUPDIR} = "${NAS_DIRS}\${UNYO_DATE}"
    if ( !(Test-Path ${ACTDAY_DB_BKUPDIR}) ) {
        FC_LogWriter ${DEF_RTNCD_NML} "SQLServerバックアップ(当日保管用NASディレクトリ)[${ACTDAY_DB_BKUPDIR}]を作成します。"
    ...
    foreach ( ${FINDFILE} in ${TG_FIND_FILES} ) {
        if ( ${FINDFILE}.PsISContainer -eq ${False} ) {
            ...
            if ( ${RTN_CPY_CODE} ) {
                ...
                FC_LogWriter ${PRG_EXIT_CODE} "SQLServerバックアップ事後処理(NAS保管処理)が異常終了しました。"
                EndProcess
            }
        }
    }
}
FC_LogWriter ${PRG_EXIT_CODE} "SQLServerバックアップ事後処理(NAS保管処理)が終了しました。"
