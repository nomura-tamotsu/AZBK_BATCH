...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 商品ファイルロード
#
# MODULE NAME   : mb_product_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
#-----+----------+---------------+-------------------------------------------
# %00 | 14/04/03 | ISID        | First Edition.                            
#============================================================================
#============================================================================
# 関数名        mb_product_load-End-Function
# 概要          終了処理
...
# 戻り値型      なし
#============================================================================
function local:mb_product_load-End-Function () {
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****商品ファイルロード開始*****"
...
$VIEW_NAME = $PRODUCT_VIEWNAME

#---------------------------------------
# 商品ファイル存在チェック
#---------------------------------------
if ( -not ( Test-Path $PRODUCT_DATA_FILE ) ) {
	# ファイル存在チェックエラーメッセージ
	. $OUTMSG_CMD $FILE_NOT_EXIST_MSG $RC_NG $PRODUCT_DATA_FILE

	$exit_code = $RC_NG
	mb_product_load-End-Function
}

#---------------------------------------
# 商品ファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $PRODUCT_DATA_FILE).Length

#　ファイルサイズが 0 バイトの場合はエラー
if ( $filesize  -eq 0 ) {
	$ADD_MSG = "ファイルサイズが 0 バイトです("+ $PRODUCT_DATA_FILE + ")"
...
$TARGET_VIEW_BASE = $SQL_VIEW_PRODUCT_BASE
if ( $ret -eq $VIEW_A ) {
...
#  ベースネームをセット
#---------------------------------------
$M_BASE_NAME = $M_PRODUCT_BASE

# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_PRODUCT_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "商品ファイルファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "商品ファイルファイルロードを終了しました．"
...
# ファイル名のみ取得
$new_file_name = split-path $PRODUCT_DATA_FILE  -Leaf
...
# Move 実行
$rc = Move-Item $PRODUCT_DATA_FILE $new_file_name -force 2>&1
$ret=$?
if ( $ret -ne "TRUE" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "ロードファイル退避($PRODUCT_DATA_FILE)でエラーが発生しました"
	$exit_code=$RC_NG
	mb_product_load-End-Function
}
