...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 世帯属性差分データ取込
#
# MODULE NAME   : mb_household_reflect
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
#-----+----------+---------------+-------------------------------------------
# %00 | 14/04/11 | ISID        | First Edition.                            
#============================================================================
#============================================================================
# 関数名        mb_household_reflect-End-Function
# 概要          終了処理
...
# 戻り値型      なし
#============================================================================
function local:mb_household_reflect-End-Function () {
...
# 処理名セット
$BATNAME  = "mb_household_reflect"
$JOB_NAME = "世帯属性差分データ取込"
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****世帯属性差分データ取込開始*****"
...
if ( -not ( Test-Path $HOUSEHOLD_DATA_FILE ) ) {
	...
	$exit_code = $RC_NG
	mb_household_reflect-End-Function
}
...
#---------------------------------------
# テーブル名
$TABLE_NAME    = $HOUSEHOLD_BASE_TBNAME
...
# 世帯属性差分データ取込実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "世帯属性差分データ取込を開始します．"

$rc=DailyHousehold $DAT_FILE $LOG_FILE
$ret=$?
#$ret="True"
if ( $ret -ne "True" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "世帯属性差分データ取込でエラーが発生しました"
	$exit_code = $RC_NG
	mb_household_reflect-End-Function

}

. $OUTMSG_CMD $INFO_MSG $RC_OK "世帯属性差分データ取込を終了しました．"
...
$ret=$?
if ( $ret -ne "TRUE" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "データファイル退避($HOUSEHOLD_DATA_FILE)でエラーが発生しました"
	$exit_code=$RC_NG
	mb_household_reflect-End-Function
}
