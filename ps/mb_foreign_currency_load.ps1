...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 外貨預金ロード
#
# MODULE NAME   : mb_foreign_currency_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
#============================================================================
#============================================================================
# 関数名        mb_foreign_currency_load-End-Function
# 概要          終了処理
...
# 戻り値型      なし
#============================================================================
function local:mb_foreign_currency_load-End-Function () {
...
# 処理名セット
$BATNAME  = "mb_foreign_currency_load"
$JOB_NAME = "外貨預金ロード"
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****外貨預金ロード開始*****"
...
$VIEW_NAME = $FOREIGN_VIEWNAME

#---------------------------------------
# 外貨預金ファイル存在チェック
#---------------------------------------
if ( -not ( Test-Path $FOREIGN_DATA_FILE ) ) {
	# ファイル存在チェックエラーメッセージ
	. $OUTMSG_CMD $FILE_NOT_EXIST_MSG $RC_NG $FOREIGN_DATA_FILE

	$exit_code = $RC_NG
	mb_foreign_currency_load-End-Function
}

#---------------------------------------
# 外貨預金ファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $FOREIGN_DATA_FILE).Length

#　ファイルサイズが 0 バイトの場合はエラー
if ( $filesize  -eq 0 ) {
	$ADD_MSG = "ファイルサイズが 0 バイトです("+ $FOREIGN_DATA_FILE + ")"
...
$TARGET_VIEW_BASE = $SQL_VIEW_FOREIGN_BASE
if ( $ret -eq $VIEW_A ) {
...
#  ベースネームをセット
#---------------------------------------
$M_BASE_NAME = $M_FOREIGN_BASE

# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_FOREIGN_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "外貨預金ファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "外貨預金ファイルロードを終了しました．"
...
# ファイル名のみ取得
$new_file_name = split-path $FOREIGN_DATA_FILE  -Leaf
...
# Move 実行
$rc = Move-Item $FOREIGN_DATA_FILE $new_file_name -force 2>&1
$ret=$?
if ( $ret -ne "TRUE" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "ロードファイル退避($FOREIGN_DATA_FILE)でエラーが発生しました"
	$exit_code=$RC_NG
	mb_foreign_currency_load-End-Function
}
