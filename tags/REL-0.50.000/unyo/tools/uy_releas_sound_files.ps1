...
if ( ${TG_FIND_FILES}.length -gt 0 ) {
    ...
    foreach ( ${FINDFILE} in ${TG_FIND_FILES} ) {
        if ( ${FINDFILE}.PsISContainer -eq ${False} ) {
            ...
            FC_LogWriter ${DEF_RTNCD_NML} "[${COPY_FILEDIFI_S}]の同期処理を開始します。"
            ${RTN_CPY_TEMP} = Copy-Item ${COPY_FILEDIFI_S} ${LC_SYNC_DEST_DIR} -Recurse
            ${RTN_CPY_CODE} = $?
            if ( ${RTN_CPY_CODE} ) {
                FC_LogWriter ${DEF_RTNCD_NML} "[${COPY_FILEDIFI_S}]の同期処理が正常終了しました。"
            } else {
                ${PRG_EXIT_CODE} = ${DEF_RTNCD_ERR}
                outmsg 55 ${PRG_EXIT_CODE} "[${COPY_FILEDIFI_S}]の同期処理にてエラーが発生しました。"
                FC_LogWriter ${PRG_EXIT_CODE} "[${COPY_FILEDIFI_S}]の同期処理にてエラーが発生しました。"
