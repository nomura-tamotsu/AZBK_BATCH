...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 定期性預金データロード
#
# MODULE NAME   : mb_term_deposit_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
#-----+----------+---------------+-------------------------------------------
# %00 | 15/01/22 | 土居 康一郎   | First Edition.
#============================================================================
#============================================================================
...
# 処理名セット
$BATNAME  = "mb_term_deposit_load"
$JOB_NAME = "定期性預金データロード"
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****定期性預金データロード開始*****"
...
$VIEW_NAME = $TERM_DEPOSIT_BASE_VIEWNAME
...
# 定期性預金データファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $TERM_DEPOSIT_DATA_FILE).Length

#　ファイルサイズが 0 バイトの場合はエラー
if ( $filesize  -eq 0 ) {
    $ADD_MSG = "ファイルサイズが 0 バイトです("+ $TERM_DEPOSIT_DATA_FILE + ")"
...
$TARGET_VIEW_BASE = $SQL_VIEW_TERM_DEPOSIT_BASE
if ( $ret -eq $VIEW_A ) {
...
#  ベースネームをセット
#---------------------------------------
$M_BASE_NAME = $M_TERM_DEPOSIT_BASE

# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_TERM_DEPOSIT_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "定期性預金データファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "定期性預金データファイルロードを終了しました．"
...
$ret=$?
if ( $ret -ne "TRUE" ) {
        . $OUTMSG_CMD $COM_ERROR $RC_NG "ロードファイル退避($TERM_DEPOSIT_DATA_FILE)でエラーが発生しました"
    $exit_code=$RC_NG
    mb_attribute_load-End-Function
}
