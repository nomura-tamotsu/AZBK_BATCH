...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 投信明細データロード
#
# MODULE NAME   : mb_fund_detail_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
# %00 | 15/01/26 | 土居 康一郎   | 1.0次対応 ローディング情報更新を追加
#============================================================================
#============================================================================
# 関数名        mb_fund_detail_load-End-Function
# 概要          終了処理
...
# 戻り値型      なし
#============================================================================
function local:mb_fund_detail_load-End-Function () {
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****投信明細データロード開始*****"
...
$VIEW_NAME = $FUND_DETAIL_VIEWNAME

#---------------------------------------
# 投信明細データファイル存在チェック
#---------------------------------------
if ( -not ( Test-Path $FUND_DETAIL_DATA_FILE ) ) {
	# ファイル存在チェックエラーメッセージ
	. $OUTMSG_CMD $FILE_NOT_EXIST_MSG $RC_NG $FUND_DETAIL_DATA_FILE

	$exit_code = $RC_NG
	mb_fund_detail_load-End-Function
}

#---------------------------------------
# 投信明細ファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $FUND_DETAIL_DATA_FILE).Length

#　ファイルサイズが 0 バイトの場合はエラー
if ( $filesize  -eq 0 ) {
	$ADD_MSG = "ファイルサイズが 0 バイトです("+ $FUND_DETAIL_DATA_FILE + ")"
...
$TARGET_VIEW_BASE = $SQL_VIEW_FUND_DETAIL_BASE
if ( $ret -eq $VIEW_A ) {
...
#  ベースネームをセット
#---------------------------------------
$M_BASE_NAME = $M_FUND_DETAIL_BASE

# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_FUND_DETAIL_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "投信明細データファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "投信明細データファイルロードを終了しました．"
...
# ファイル名のみ取得
$new_file_name = split-path $FUND_DETAIL_DATA_FILE  -Leaf
...
# Move 実行
$rc = Move-Item $FUND_DETAIL_DATA_FILE $new_file_name -force 2>&1
$ret=$?
if ( $ret -ne "TRUE" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "ロードファイル退避($FUND_DETAIL_DATA_FILE)でエラーが発生しました"
	$exit_code=$RC_NG
	mb_fund_detail_load-End-Function
}
