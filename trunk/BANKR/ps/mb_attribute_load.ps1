...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 顧客属性データロード
#
# MODULE NAME   : mb_attribute_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
#-----+----------+---------------+-------------------------------------------
# %00 | 15/01/14 | 土居 康一郎   | First Edition.                            
#============================================================================
#============================================================================
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****顧客属性データロード開始*****"
...
$VIEW_NAME = $CUSTOMER_BASE_VIEWNAME
...
# 顧客属性データファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $CUSTOMER_DATA_FILE).Length

#　ファイルサイズが 0 バイトの場合はエラー
if ( $filesize  -eq 0 ) {
	$ADD_MSG = "ファイルサイズが 0 バイトです("+ $CUSTOMER_DATA_FILE + ")"
...
$TARGET_VIEW_BASE = $SQL_VIEW_CUSTOMER_BASE
if ( $ret -eq $VIEW_A ) {
...
# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_CUSTOMER_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "顧客属性データファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "顧客属性データファイルロードを終了しました．"
...
$ret=$?
if ( $ret -ne "TRUE" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "ロードファイル退避($CUSTOMER_DATA_FILE)でエラーが発生しました"
	$exit_code=$RC_NG
	mb_attribute_load-End-Function
}
