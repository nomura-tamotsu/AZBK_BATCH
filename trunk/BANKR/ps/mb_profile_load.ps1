...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 顧客プロファイルデータロード
#
# MODULE NAME   : mb_profile_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
#-----+----------+---------------+-------------------------------------------
# %00 | 15/01/27 | 土居 康一郎   | First Edition.                            
#============================================================================
#============================================================================
...
# 処理名セット
$BATNAME  = "mb_profile_load"
$JOB_NAME = "顧客プロファイルデータロード"
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****顧客プロファイルデータロード開始*****"
. $OUTMSG_CMD $START_MSG $RC_OK $JOB_NAME

#---------------------------------------
# 顧客プロファイルデータファイル名をセット
#---------------------------------------
$PROFILE_DATA_FILE = "$EXT_LOAD_DIR\" + $M_PROFILE_BASE + ".dat"
$VIEW_NAME = $PROFILE_VIEWNAME

#---------------------------------------
# 顧客プロファイルデータファイル存在チェック
#---------------------------------------
if ( -not ( Test-Path $PROFILE_DATA_FILE ) ) {
	# ファイル存在チェックエラーメッセージ
	. $OUTMSG_CMD $FILE_NOT_EXIST_MSG $RC_NG $PROFILE_DATA_FILE
...
#---------------------------------------
# 顧客プロファイルデータファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $PROFILE_DATA_FILE).Length

#　ファイルサイズが 0 バイトの場合はエラー
if ( $filesize  -eq 0 ) {
	$ADD_MSG = "ファイルサイズが 0 バイトです("+ $PROFILE_DATA_FILE + ")"
...
$TARGET_VIEW_BASE = $SQL_VIEW_PROFILE_BASE
if ( $ret -eq $VIEW_A ) {
...
#  ベースネームをセット
#---------------------------------------
$M_BASE_NAME = $M_PROFILE_BASE

# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_PROFILE_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "顧客プロファイルデータファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "顧客プロファイルデータファイルロードを終了しました．"
...
# ファイル名のみ取得
$new_file_name = split-path $PROFILE_DATA_FILE  -Leaf
...
# Move 実行
$rc = Move-Item $PROFILE_DATA_FILE $new_file_name -force 2>&1
$ret=$?
if ( $ret -ne "TRUE" ) {
       	. $OUTMSG_CMD $COM_ERROR $RC_NG "ロードファイル退避($PROFILE_DATA_FILE)でエラーが発生しました"
	$exit_code=$RC_NG
	mb_attribute_load-End-Function
}
