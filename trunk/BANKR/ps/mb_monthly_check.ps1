...
function Endfile-Rename () {
	#
	# 配置完了（月次）ファイルを退避（ファイル名.YYYYMMDDHHMISSの形式で退避ディレクトリへ）
	#
	...
	$ret=$?
	if ( $ret -ne "TRUE" ) {
       		. $OUTMSG_CMD $COM_ERROR $RC_NG "配置完了（月次）ファイル退避でエラーが発生しました"
		$exit_code=$RC_NG
...
#============================================================================
# 関数名        mb_monthly_check-End-Function
# 概要          終了処理
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****月次処理チェック処理*****"
...
if ( -not ( Test-Path $MONTHLY_END_FILE ) ) {
	# 月次処理日ではないメッセージ
       	. $OUTMSG_CMD $NOT_EXECUTE_DAY $RC_OK "月次"
	...
	# 月次処理日のメッセージ
       	. $OUTMSG_CMD $EXECUTE_DAY $RC_OK "月次"
