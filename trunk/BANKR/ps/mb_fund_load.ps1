...
# SYSTEM        : BANK・R－CC
#
# PROGRAM NAME  : 投信データロード
#
# MODULE NAME   : mb_fund_load
#
# INPUT         : None
#
# EXIT          : 正常終了 ・・・ NORMAL(0)
...
# 開始メッセージ
. $OUTMSG_CMD $INFO_MSG $RC_OK "*****投信データロード開始*****"
...
# 投信データファイルサイズチェック
#---------------------------------------
$filesize=$(Get-ChildItem $FUND_DATA_FILE).Length
...
#  ベースネームをセット
#---------------------------------------
$M_BASE_NAME = $M_FUND_BASE

# コントロールファイル
$CTL_FILE      = $CTL_FILE_DIR + "\M_FUND_" + $TARGET_SUFFIX + ".ctl"
...
# SQLロード実行
#---------------------------------------
. $OUTMSG_CMD $INFO_MSG $RC_OK "投信データファイルロードを開始します．"
...
. $OUTMSG_CMD $INFO_MSG $RC_OK "投信データファイルロードを終了しました．"
...
# ファイル名のみ取得
$new_file_name = split-path $FUND_DATA_FILE  -Leaf
