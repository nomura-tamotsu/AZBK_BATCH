...
CREATE OR REPLACE VIEW TF_個別明細投信口座
 ...
 AS SELECT 
    ...
    ,TF_個別明細投信口座A.利用金額
    ,TF_個別明細投信口座A.余裕枠
    ,TF_個別明細投信口座A.口座開設年月日
    ,TF_個別明細投信口座A.残高
    ,TF_個別明細投信口座A.口座区分
    ,TF_個別明細投信口座A.乗換優遇残金額
    ,TF_個別明細投信口座A.適用終了日
    ,TF_個別明細投信口座A.データ基準日
 FROM TF_個別明細投信口座A
;
COMMENT ON COLUMN TF_個別明細投信口座.店番 IS 'TM_営業店.店番を参照。';
COMMENT ON COLUMN TF_個別明細投信口座.CIFNO IS '有効桁数10桁（固定長）';
COMMENT ON COLUMN TF_個別明細投信口座.世帯番号 IS '有効桁数10桁（固定長）';
COMMENT ON COLUMN TF_個別明細投信口座.個人企業番号 IS '個人企業番号："99999999999"固定　【未使用】';
COMMENT ON COLUMN TF_個別明細投信口座.口座番号 IS '7桁固定（0パディング）';
...
COMMENT ON COLUMN TF_個別明細投信口座.振込銀行ID IS '振込銀行コード　4桁固定';
COMMENT ON COLUMN TF_個別明細投信口座.振込支店ID IS '振込支店コード　3桁固定';
COMMENT ON COLUMN TF_個別明細投信口座.科目ID IS '1:普通、2:当座、4:貯蓄預金、9:その他';
COMMENT ON COLUMN TF_個別明細投信口座.振込口座 IS '振込口座　7桁固定';
COMMENT ON COLUMN TF_個別明細投信口座.当座貸越 IS '引落口座 999-9-9999999 (店番3桁固定（0パディング） + "-" + 科目1桁固定（No13の科目） + "-" + 口座番号7桁固定(0パディング) + "-")';
...
COMMENT ON COLUMN TF_個別明細投信口座.口座区分 IS '0：未登録（一般口座）　1：源泉徴収　2：確定申告　3：NISA＋未登録　4：NISA＋源泉徴収　5：NISA＋確定申告';
