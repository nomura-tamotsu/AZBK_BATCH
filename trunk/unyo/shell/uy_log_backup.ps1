...
if ( ${PRM_FilePATH} -ne "x" ) {
    ${BACKUP_OTHER_LOG_DEF} = ${PRM_FilePATH}
}

#--------------------------------------------
# 各種ログバックアップ処理(Phase.01-COPY処理)
#--------------------------------------------
${LC_STEPMSG} = "Phase.01-COPY処理"
FC_LogWriter ${DEF_RTNCD_NML} "${PRG_NAME}(${LC_STEPMSG})を実施します。"
try {
    #FC_BACKUP_COPY ${BACKUP_OTHER_LOG_DEF} "COPY" 2>&1 > $null
    #${RTN_FCBACKUP_CODE} = $?
    ${RTN_FCBACKUP_CODE} = FC_BACKUP_COPY ${BACKUP_OTHER_LOG_DEF} "COPY" "${PRM_ReRunDay}"
...
#--------------------------------------------
# 各種ログバックアップ処理(Phase.02-MOVE処理)
#--------------------------------------------
${LC_STEPMSG} = "Phase.02-MOVE処理"
FC_LogWriter ${DEF_RTNCD_NML} "${PRG_NAME}(${LC_STEPMSG})を実施します。"
try {
    #FC_BACKUP_COPY ${BACKUP_OTHER_LOG_DEF} "MOVE" 2>&1 > $null
    #${RTN_FCBACKUP_CODE} = $?
    ${RTN_FCBACKUP_CODE} = FC_BACKUP_COPY ${BACKUP_OTHER_LOG_DEF} "MOVE" "${PRM_ReRunDay}"
    if ( !(${RTN_FCBACKUP_CODE}) ) {
        ${PRG_EXIT_CODE}    = ${DEF_RTNCD_ERR}
        FC_LogWriter ${PRG_EXIT_CODE} "${PRG_NAME}(${LC_STEPMSG})にて異常が発生しました。"
        EndProcess
